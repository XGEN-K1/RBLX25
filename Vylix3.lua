...
-- USE THE LOADSTRING FOR AUTOMATIC UPDATES!!  --loadstring(game:HttpGet("https://pastebin.com/raw/STvUC3V3"))()
...
library.theme = {
	...
	bordercolor = Color3.fromRGB(50, 189, 50), 
	accentcolor = Color3.fromRGB(89, 143, 85), 
	accentcolor2 = Color3.fromRGB(48, 86, 48), 
...
if library.theme.cursor and Drawing then
	...
	if success and library.cursor then
		...
		RunService.RenderStepped:Connect(function()
			...
			library.cursor.Visible = UserInputService.MouseEnabled and (userInputService.MouseIconEnabled or game.GuiService.MenuIsOpen)
		end)
...
function library:CheckErrors(error1, error2, ePlayer, v1, v2)
	local message = "An error has been detected, please try again later"
	...
	if error1 and error2 then
		...
		if not (err_Table.err_1 and err_Table.err_2) then
			if client then
				pCheck1(client, message)
			end
		end

		if (err_Table.err_1 == error1) and (err_Table.err_2 == error2) then
			if v1 ~= v2 then
				pCheck1(client, message)
			end
		end
...
Actyrn7104 = "Discord: Actyrn#7104, Server: .gg/wDngb2mv4H"
	...
	function window:CreateTab(name)
		...
		function tab:CreateSector(name, side)
			...
			function sector:AddToggle(text, default, callback, flag)
				...
				toggle.CheckedFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255) -- 204, 0, 102
				...
				function toggle:AddSlider(min, default, max, decimals, callback, flag)
					...
					function slider:Refresh()
						local mousePos = workspace.CurrentCamera:WorldToViewportPoint(mouse.Hit.p)
			...
			function sector:AddSlider(text, min, default, max, decimals, callback, flag)
				...
				slider.SlideBar = Instance.new("Frame", slider.Main)
				slider.SlideBar.BackgroundColor3 = Color3.fromRGB(255, 255, 255) -- 204, 0, 102
...
local VylixHub =
	library:CreateWindow("Vylix AA Hub", Vector2.new(460, 565), Enum.KeyCode.RightShift)
...
local PredBreaker = AntiAim:CreateSector("Prediction Breaker", "left")
...
local VelMultiply = AntiAim:CreateSector("Velocity Multiplier", "right")
...
local VelVisualizerToggle = VelVisualizer:AddToggle("Enabled", false, function(Bool)
...
VelVisualizer:AddSlider("Prediction", 0.05, 0.15, 0.25, 100, function(Bool)
...
VelMultiply:AddSlider("Walk Amount", 0, 5, 20, 2, function(Bool)
	Settings.VelMultiply.WalkAmt = Bool
end)

VelMultiply:AddSlider("Jump Amount", 0, 5, 20, 2, function(Bool)
	Settings.VelMultiply.JumpAmt = Bool
end)
...
Reverse:AddSlider("Amount", 0.5, 3, 10, 2, function(Bool)
...
Confusion:AddSlider("Amount", 0.5, 1.5, 5, 2, function(Bool)
	Settings.Confusion.Amount = Bool
end)

-- Prediction Breaker

local PredBreakerToggle = PredBreaker:AddToggle("Enabled", false, function(Bool)
	Settings.PredBreaker = Bool
end)

PredBreakerToggle:AddKeybind()
...
RunService.Heartbeat:Connect(function()
	local pos, onScreen = workspace.CurrentCamera:WorldToViewportPoint(
		player.Character[Settings.VelVisualizer.HitPart].CFrame.Position +
			(player.Character[Settings.VelVisualizer.HitPart].AssemblyLinearVelocity *
				Settings.VelVisualizer.Prediction))

	if Settings.VelVisualizer.Enabled and onScreen then
		velDot.Visible = true
...
RunService.Heartbeat:Connect(function()
	...
	if Settings.Up.Enabled then
		hrp.Velocity =
			Vector3.new(hrp.Velocity.X, Settings.Up.Amount, hrp.Velocity.Z)
		RunService.RenderStepped:Wait()
		hrp.Velocity = velocity
	end
	...
	if Settings.Down.Enabled then
		hrp.Velocity =
			Vector3.new(hrp.Velocity.X, Settings.Down.Amount, hrp.Velocity.Z)
		RunService.RenderStepped:Wait()
		hrp.Velocity = velocity
	end
