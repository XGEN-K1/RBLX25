...
local targetPosition = Vector3.new(2085, 27, 864) -- Точка призначення для телепортації
...
local teleportLocalOwner = false  -- Телепортувати речі локального гравця
local teleportNilOwner = false     -- Телепортувати речі, якщо власник відсутній (nil або немає властивості Owner)
...
local function teleportObjects()
    ...
    while tick() < endTime do
        for _, child in ipairs(parentFolder:GetChildren()) do
            if tick() >= endTime or teleportedCount >= maxTeleportCount then
                return
            end
            ...
            if child:IsA("Model") then
                ...
                elseif owner == playerName then
                    if teleportLocalOwner then
                        shouldTeleport = true
                    end
                elseif otherOwnerToTeleport ~= "" and owner == otherOwnerToTeleport then
                    shouldTeleport = true
                end
