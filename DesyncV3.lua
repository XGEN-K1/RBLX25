...
--// Desync Settings
local desyncEnabled = true -- Включити/виключити десинхронізацію
local desyncIntensity = 180 -- Інтенсивність десинхронізації (кути обертання)
local desyncVelocity = Vector3.new(1, 1, 1) * 16384 -- Швидкість десинхронізації
...
BruhXD.Heartbeat:Connect(function()
    if PastedSources == true and desyncEnabled then
        ...
        SpoofThis = SpoofThis * CFrame.Angles(math.rad(RandomNumberRange(desyncIntensity)), math.rad(RandomNumberRange(desyncIntensity)), math.rad(RandomNumberRange(desyncIntensity)))

        LocalPlayer.Character.HumanoidRootPart.CFrame = SpoofThis

        LocalPlayer.Character.HumanoidRootPart.AssemblyLinearVelocity = desyncVelocity
