local Settings = {
    rewrittenmain = {
        ...
        TriggerKey = "h", -- Тригерна кнопка (H), що активує/деактиває режим стрільби ЛКМ
        ...
        wallcheck = false, -- Перевірка на перешкоди (стіни) між tool і ціллю
...
local function disableLocalScriptsInTool(tool)
    if disabledToolScripts[tool] then return end
...
function TargetNotif()
    if Plr then
        ...
        local FinalNotifString = tostring(" Name: " .. tostring(Plr.Name) ..
...
mouse.KeyUp:Connect(function(k)
    if k == Settings.rewrittenmain.ShootKey and Settings.rewrittenmain.UseVKey and not triggerMode then
        shooting = false
    end
end)
